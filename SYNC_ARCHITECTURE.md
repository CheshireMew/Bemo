...
Web / Desktop 的主存储是后端 `app.db`；Mobile 使用本地 IndexedDB。同步算法由前端共享，写入通过当前运行时的存储适配器完成。不能把 Web / Desktop 的浏览器缓存当作主存储，也不能假设后端失联后仍可离线写笔记。

后端笔记变更与待同步记录在同一个 SQLite 事务中提交。远端变更先经过前端冲突判断，再通过带快照校验的事务接口写入后端；处理回执与数据一起提交，使浏览器游标写入失败后的重放可以恢复。Mobile 的 IndexedDB 请求统一在事务提交后才报告成功，失败会传回界面。

同步数据库 `sync.db` 只负责同步协议，不再把远端写入隐式投影到 `app.db`。推送完成后从上次已处理游标拉取，不能用推送返回的新游标跳过同时到达的远端变更。附件写入各端自己的主存储；后台待同步记录引用的附件不会被手动清理入口当作孤立文件。

备份和 Markdown 恢复属于替换当前主存储的操作：先校验完整内容，暂停本窗口同步，再进行事务替换。重置设备只清理本机状态，不清空 Web / Desktop 后端。其它窗口仍在写入时，应先关闭它们再恢复备份。具体入口见 [运维说明](docs/operations.md) 和 [当前架构](CURRENT_ARCHITECTURE.md)。

## 历史设计草案（非当前验收依据）

以下保留早期方案供追溯，其中“所有平台本地优先”、SQLite 平台建议、接口草案和阶段计划不再代表当前实现。
