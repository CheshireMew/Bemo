...
export function getAiConfigurationIssue() {
  if (!settings.ai.enabled) {
    return 'AI 功能尚未启用。';
  }
  if (!normalizeBaseUrl(settings.ai.baseUrl) || !settings.ai.model.trim() || !settings.ai.hasApiKey) {
    return 'AI 配置不完整，请补全接口地址、API Key 和模型。';
  }
  return '';
}
