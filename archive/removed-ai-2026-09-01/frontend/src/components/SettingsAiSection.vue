<template>
  <section class="settings-section">
    <div class="section-header">
      <div>
        <h3>AI</h3>
        <p>配置兼容 OpenAI API 的服务后，即可使用 AI 对话和笔记问答。</p>
      </div>
    </div>
    ...
    <div class="settings-card form-card">
      <label class="toggle-row">
        <div>
          ...
          <span class="field-hint">关闭后隐藏 AI 入口，已保存的配置和对话仍会保留。</span>
        </div>
...
const persistAiSettings = async () => {
  try {
    ...
    pushNotification('AI 配置已保存', 'success');
  } catch (error) {
...
@container settings (max-width: 640px) {
  ...
  .field-row input, .field-row select, .field-input-stack { width: 100%; min-width: 0; }
}
