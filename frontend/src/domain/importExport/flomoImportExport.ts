...
import { buildLocalAttachmentPath, createLocalAttachmentFilename } from '../../utils/localAttachments.js';
...
async function parseFlomoZip(file: File) {
  ...
  for (const [index, block] of memoBlocks.entries()) {
    ...
    for (const match of attachmentMatches) {
      ...
      if (seenAttachmentPaths.has(attachmentPath)) continue;
    ...
    notes.push({
      ...
      title: content.split('\n')[0].replace(/^#+\s*/, '').slice(0, 40) || 'Flomo 导入',
      created_at: ts,
      updated_at: ts,
      content,
      tags: [],
...
export async function importFlomoArchive(file: File) {
  ...
  if (isZipFile(file)) {
    ...
    if (lines.length <= 1) {
      return { imported_count: 0, imported_attachment_count: 0, imported_note_records: [] };
    }
  ...
  await enqueueImportedNotes(importedNoteRecords);
