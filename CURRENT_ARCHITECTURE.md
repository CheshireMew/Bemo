...
Web / Desktop 笔记写入和待同步记录共同落在后端 `app.db`。共享同步逻辑通过运行时适配器应用远端变更；后端 `sync.db` 不代替应用主存储。浏览器仅保留可重建的阅读缓存、设备状态和冲突展示状态。Mobile 的主存储仍为本地 IndexedDB，不因此改为后端存储。

桌面应用启动自己管理的 Python 后端，等待就绪地址后才挂载产品界面；退出时关闭后端。Web 网络部署由同源代理连接后端并对整个应用鉴权。环境准备与数据目录见 [运维说明](docs/operations.md)。
